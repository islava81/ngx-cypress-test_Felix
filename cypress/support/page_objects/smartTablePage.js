...
export class SmartTable{
    updateAgeByFirstName(name, age){
        cy.get('tbody').contains('tr', name).then(tableRow => {
            ...
            cy.wrap(tableRow).find('[placeholder="Age"]').clear().type(age) //If the "clear" is not added, both the original and newly
            //inserted age values will be included in the input field
            cy.wrap(tableRow).find('[class="nb-checkmark"]').click( {force:true} ) //This could have also been put in as ".nb-checkmark"
